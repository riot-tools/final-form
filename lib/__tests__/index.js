...
global.MutationObserver = function MutationObserver(callback) {
    ...
    stub.mutator.instantiated(callback);
    Object.assign(this, stub.mutator);

    // Optionally add a trigger() method to manually trigger a change
    stub.mutator.trigger = (mockedMutationsList) => {
...
test('it capture form mutations', () => {
    ...
    expect(stub.mutator.instantiated).toHaveBeenCalled();
    expect(stub.mutator.observe).toHaveBeenCalled();

    const mockedMutationsList = {};

    stub.mutator.trigger(mockedMutationsList);
    expect(stub.mocks.formMutated).toHaveBeenCalled();

    const [[arg]] = stub.mocks.formMutated.mock.calls;
    ...
    expect(stub.mutator.disconnect).not.toHaveBeenCalled()

    tmpl.unmount();

    expect(stub.mutator.disconnect).toHaveBeenCalled()
